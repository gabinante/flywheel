...
export function createResidualService(deps: ResidualServiceDeps) {
  const { prisma, nmiClient, logger } = deps;
...
async function logAuditEntry(
  ...
  try {
    ...
    // AuditLog table may not exist yet — use structured logger as fallback
    // Note: logger is not available in this static helper; callers should
    // handle audit log failures in their own context.
  }
}
