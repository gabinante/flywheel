...
export async function submitBoardingApplication(
  ...
  console.log(
    `[NMI Boarding] Submitting application for merchant ${input.merchantId}, ` +
      `application ${input.applicationId}, business: ${input.businessLegalName}`
  );
  ...
  try {
    ...
    console.error(
      `[NMI Boarding] API request failed for application ${input.applicationId}:`,
  ...
  console.log(
    `[NMI Boarding] Response for application ${input.applicationId}: ` +
  ...
  console.warn(
    `[NMI Boarding] Unknown response status for application ${input.applicationId}: ` +
      `${responseStatus}. Treating as UNDER_REVIEW.`
  );
  return {
    success: true,
    nmiApplicationId: parsed.boarding_id || parsed.application_id || parsed.id,
...
export async function checkBoardingStatus(
  ...
  console.log(
    `[NMI Boarding] Checking status for NMI application ${nmiApplicationId}`
  );
  ...
  try {
    ...
    console.error(
      `[NMI Boarding] Status check failed for ${nmiApplicationId}:`,
      err instanceof Error ? err.message : "Unknown error"
    );
    throw err;
  }
  ...
  console.log(
    `[NMI Boarding] Status check result for ${nmiApplicationId}: ` +
      `status=${parsed.status || parsed.response_code || "unknown"}`
  );
