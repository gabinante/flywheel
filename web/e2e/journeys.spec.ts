...
test('J09 define a worker and assign it to reviews and implementation', async ({ page }, info) => {
  ...
  try {
    ...
    await page.getByLabel('Additional instructions', { exact: true }).fill('Keep reviews pragmatic.')
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await expect(page.getByText('Saved', { exact: true })).toBeVisible()
    await page.reload()
    await page.getByRole('button', { name: /Audit worker/ }).click()
    await expect(page.getByLabel('Additional instructions', { exact: true })).toHaveValue('Keep reviews pragmatic.')
    await expect(page.getByRole('combobox', { name: 'Model', exact: true })).toHaveText('gpt-5.6-sol')
    await expect(page.getByRole('combobox', { name: 'Reasoning effort', exact: true })).toHaveText('high')
    await page.getByRole('combobox', { name: 'Harness', exact: true }).click()
    await page.getByRole('option', { name: 'Claude Code', exact: true }).click()
    await expect(page.getByRole('combobox', { name: 'Model', exact: true })).toHaveText('Harness default')
    await expect(page.getByRole('combobox', { name: 'Reasoning effort', exact: true })).toHaveText('Harness default')
    await page.getByRole('combobox', { name: 'Model', exact: true }).click()
    ...
    await expect(page.getByLabel('Additional instructions', { exact: true })).toBeVisible()
...
test('J20 edit an included worker through the same worker editor', async ({ page }) => {
  const saved = await json('GET', '/settings')
  const prompt = (await json('GET', '/prompts')).items.find((p: { id: string }) => p.id === 'code_review')
  try {
    await page.goto('/#/settings?section=workers')
    ...
    await page.getByRole('combobox', { name: 'Model', exact: true }).click()
    await page.getByRole('option', { name: 'gpt-5.6-sol', exact: true }).click()
    await page.getByLabel('Additional instructions', { exact: true }).fill('Focus on correctness and keep feedback short.')
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await expect(page.getByText('Saved', { exact: true }).first()).toBeVisible()
    await page.reload()
    await page.getByRole('button', { name: /^Pragmatic reviewer/ }).click()
    await expect(page.getByRole('textbox', { name: 'Code reviewer prompt', exact: true })).toHaveValue(/Keep examples concrete/)
    await expect(page.getByLabel('Additional instructions', { exact: true })).toHaveValue('Focus on correctness and keep feedback short.')
    await expect(page.getByRole('combobox', { name: 'Model', exact: true })).toHaveText('gpt-5.6-sol')
    await page.getByRole('tab', { name: 'Task assignments' }).click()
    ...
    await json('PUT', '/prompts/code_review', { text: prompt.customized ? prompt.text : '' })
  }
})
