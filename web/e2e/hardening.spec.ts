...
test('the global tray keeps cross-project decisions and navigation on every page', async ({ page }) => {
  ...
  for (const path of ['/settings', '/workflows', `/orgs/${org.slug}/projects/${empty.slug}/command`, `/orgs/${org.slug}/projects/${project.slug}/tickets/${gateTicket}`]) {
    ...
    await expect(tray.getByRole('group', { name: 'Reviewer status' })).toContainText('workers')
    await expect(tray.getByRole('group', { name: 'Reviewer status' }).getByRole('link')).toHaveAttribute('href', '#/code-reviews')
...
test('tray errors preserve the last known work and the tray opens on narrow screens', async ({ page }) => {
  ...
  await expect(reviewers).toContainText('workers')
  const lastReviewerStatus = await reviewers.getByRole('link').innerText()
  await page.route('**/api/overview', route => route.fulfill({ status: 503, body: 'unavailable' }))
  await page.route('**/code-reviews/status', route => route.fulfill({ status: 503, body: 'unavailable' }))
  await tray.getByRole('button', { name: 'Refresh current work' }).click()
  await expect(tray.getByRole('alert').filter({ hasText: 'Could not refresh current work' })).toContainText('Showing the last successful update')
  await expect(reviewers.getByRole('alert')).toContainText('Showing the last successful update')
  await expect(reviewers.getByRole('link')).toHaveText(lastReviewerStatus, { useInnerText: true })
  await expect(tray.locator(`[data-work-id="ticket:${gateTicket}"]`)).toBeVisible()
...
test('real dispatcher phases drive both harness protocols, exact claims, session capture, and approval', async ({ page }) => {
  ...
  try {
    ...
    for (const harness of ['claude', 'codex']) {
      ...
      await expect(row.getByRole('link').filter({ hasText: ticket.title })).toHaveAttribute('href', `#/orgs/${org.slug}/projects/${workerProject.slug}/tickets/${ticket.id}`)
