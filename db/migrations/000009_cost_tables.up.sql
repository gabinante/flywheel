...
CREATE INDEX idx_llm_calls_project_time ON llm_call_records (project_id, created_at);
CREATE INDEX idx_llm_calls_ticket ON llm_call_records (ticket_id);
CREATE INDEX idx_llm_calls_project_month ON llm_call_records (project_id, date_trunc('month', created_at));
...
CREATE UNIQUE INDEX idx_budgets_scope ON budgets (project_id, ticket_id, month);
...
CREATE INDEX idx_budget_alerts_project_active ON budget_alerts (project_id) WHERE NOT acknowledged;
...
CREATE INDEX idx_rate_limits_project_active ON rate_limit_events (project_id) WHERE NOT resumed;
