...
func TestRenderProjectUpdateAndHealth(t *testing.T) {
	d := ProjectData{
		...
		Done:        []TicketLine{{Identifier: "RLEP-3626", Title: "Environment setting", URL: "https://linear.app/x/issue/RLEP-3626", State: "closed"}},
		MergedPRs:   []PRLine{{Repo: "joinhandshake/joinera", Number: 21017, Title: "Make the setting", URL: "https://github.com/joinhandshake/joinera/pull/21017"}},
		Blocked:     []TicketLine{{Identifier: "RLEP-3631", Title: "Loss mode"}},
		Activity:    []HarnessLine{{Harness: "codex", Sessions: 3, TokensIn: 1_200_000, TokensOut: 8_000}},
	}
	body := RenderProjectUpdate(d)
	for _, want := range []string{"## Update — Sep 4", "**Done (1)**", "[RLEP-3626](https://linear.app/x/issue/RLEP-3626)", "**Merged (1)**", "[joinera#21017]", "**Blocked or waiting on a decision (1)**", "Codex: 3 sessions, 1.2M in / 8k out"} {
...
func TestRenderWeeklyRoundupAndPrepend(t *testing.T) {
	...
	w := WeekData{Start: start, End: end,
		MergedPRs: []PRLine{{Repo: "joinhandshake/rle-data-processing", Number: 369, Title: "deid: check | output", URL: "u", When: start.AddDate(0, 0, 2)}},
		Projects:  []ProjectData{{ProjectName: "Buckeye", InReview: []TicketLine{{Identifier: "RLEP-3634", Title: "Launch briefs"}}}},
	}
	section := RenderWeeklyRoundup(w)
	for _, want := range []string{"## Week of " + start.Format("Jan 2, 2006"), "### Shipped (1 PRs merged)", "deid: check \\| output", "### Buckeye", "| RLEP-3634 | Launch briefs | In review |"} {
