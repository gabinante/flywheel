...
// ResolveForProject returns the effective workflow ID and first phase ID for a project.
// Returns empty strings if no active workflow exists.
func (r *Resolver) ResolveForProject(ctx context.Context, orgID, projectID string) (workflowID, firstPhaseID string, err error) {
	...
	return def.ID, def.Phases[0].ID, nil
}
